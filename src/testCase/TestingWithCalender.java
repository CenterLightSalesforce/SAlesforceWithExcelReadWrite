...
public class TestingWithCalender {
	...
	//	System.out.println("Enter Data for Srearh");
		...
		String accNa="Mark A Jara";
		driver.manage().window().maximize();
		
		List<WebElement> allEdit=driver.findElements(By.xpath(".//*[@id='Account_body']/table/tbody/tr"));
		System.out.println(allEdit.size()-1);
			
		
		for(int x=1;x<allEdit.size();x++){
			//WebElement tab=driver.findElements(By.tagName("li")).get(x);
			WebElement nameLink=driver.findElement(By.xpath(".//*[@id='Account_body']/table/tbody/tr["+x+"]/th/a"));
			String getName=nameLink.getText();
			
			if(getName.endsWith(accNa)){
				WebElement accType=driver.findElement(By.xpath(".//*[@id='Account_body']/table/tbody/tr["+x+"]/td[6]"));
				String accTypeRec=accType.getText();
				   if(accTypeRec.endsWith("Referral Source")){
				    nameLink.click();
				    Thread.sleep(2000);
				    List<WebElement> alldata=driver.findElements(By.xpath(".//tr/td[starts-with(@id,'00NF000000')]"));
				     for(int m=0;m<alldata.size();m++){
				    	 WebElement alldata1=driver.findElements(By.xpath(".//tr/td[starts-with(@id,'00NF000000')]")).get(m);
				    	String printData= alldata1.getText();
				    	System.out.println("TextBox Number:"+m);
		...
		/*driver.manage().window().maximize();
		List<WebElement> leftRecentItem=driver.findElements(By.className("mruText"));
		List<WebElement> allTab=driver.findElements(By.tagName("li"));
		//List<WebElement> allTab=driver.findElements(By.xpath("li[ends-with(@id,'Tab']"));
		for(int x=0;x<allTab.size();x++){
			WebElement tab=driver.findElements(By.tagName("li")).get(x);
			//WebElement tab=driver.findElements(By.xpath("li[ends-with(@id,'Tab']")).get(x);
		...
		for(int j=0;j<leftRecentItem.size();j++){
			WebElement obj=driver.findElements(By.className("mruText")).get(j);
			String text=obj.getText();
			System.out.println(text);
			
			if(text.equals(srearchData)){
				obj.click();
			    con=1;
				continue;
			}
		...
		if(con==0){
			System.out.println(srearchData+" Data Didnt found on The List");	
		}else{
			System.out.println("Record Match.Record open :"+srearchData);	
		}
		*/
		...
		for(int i=1;i<=list.size();i++){
			...
			List<WebElement> getData=driver.findElements(By.xpath(".//*[@id='actionForm']/div[2]/table/tbody/tr["+row+"]/th"));
			    ...
			    if(d.getText().equals("11/20/2015")){
			    	Thread.sleep(2000);
			    	 accName=driver.findElement(By.xpath(".//*[@id='actionForm']/div[2]/table/tbody/tr["+row+"]/td[5]/a")).getText();
			    	   //                                     .//*[@id='actionForm']/div[2]/table/tbody/tr[8]/td[5]/a
